(function(){
	...
	function createSubject(count){
		...
		if(count < len){
			...
			if(count == 0){
				html += '<input type="text" id="curValue_'+count+'" class="input">';//请输入大于0的任意数值
				html += ' <label>元</label>';
				html += '<div class="tip error" id="errorTip_'+count+'"></div>';
			} else if(count == 1){
				html += '<input type="text" id="curValue_'+count+'" class="input">';
				html += ' <label>元</label>';
				html += '<div class="tip error" id="errorTip_'+count+'"></div>';
			} else if(count == 2){
				html += '<select id="curValue_'+count+'">';
				for(var i=0; i<=20; i++){
	...
	function worthResult(ret,len){
		...
		var pSum = P + P2;
		var sum = webUtil.numFormat(pSum, -1); //价值
		var grow = webUtil.numFormat(((pSum - curStore) / curStore) * 100, -1); //增长
		var rank;
		if(pSum < 28000){
			rank = webUtil.numFormat((pSum / 560) + 1, -1);
		} else if(pSum > 28000 && pSum < 200000){
			rank = webUtil.numFormat((pSum - 28000) / 3822 + 50, -1);
		} else if(pSum > 200000 && pSum < 1000000){
			rank = webUtil.numFormat((pSum - 200000) / 200000 + 95, -1);
		} else if(pSum > 1000000){
			rank = 99
		};
		worthCharting(sum, len);
		var html = '<li>坚持在微积金投资，'+estTime+'年后您的身价将为<span class="light-org">'+sum+'</span>元，增长<span class="light-org">'+grow+'%</span></li>';
