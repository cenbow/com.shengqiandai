...
<body>
			...
			<div class="left-side-bottom">
				...
				<table width="542" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td width="432">${b.style==0?'等额本息':(b.style==1?'等本等息':(b.style==2?'到期还款':'先息后本'))}</td>
					</tr>
					<tr>
		...
		<div class="right-side fr">
			...
			<div class="login">
				...
				<div class="login-right fr">
					...
					<form action="#" method="post" id="loginForm" class="ui-form">

						<div class="ui-form-item">
							<label class="ui-label">用户名：</label> <input type="text"
								id="username" class="ui-input" name="username"
								value="${reg.phone}" /> <input type="hidden" id="returnUrl"
								value="${returnUrl}" /> <input type="hidden" id="third"
								value="${reg.uaccount}" /> <input type="hidden" id="bid"
								value="${reg.thirdproductid}" /> <input type="hidden"
								id="errorNum" value="1" /> <input type="hidden"
								id="showController" value="${showVerifyCode}" />
						...
						<div class="ui-form-item">
							...
							<input type="hidden" value="" id="errorNum">
						</div>

						<div class="ui-form-item codeBox mt20"
							style="display: none; height: 40px; width: 302px;">
							<label class="ui-label">验证码：</label> <input type="text"
								id="authCode" name="authCode" maxlength="4" size="10" /> <a
								href="javascript:changeImage();"> <img id="authCodeImg"
								alt="看不清?换一张"
						...
						<div class="ui-form-item" style="margin-top:14px;padding-left:74px; padding-bottom: 0px;">注：红包活动新用户【初始登录密码】</div>
						<div class="ui-form-item" style="margin-top:0px;padding-left:74px;">请查看短信，或财经道网站站内信</div>
					</form>
				</div>
			</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			wrongHtml = new Array("", "请输入用户名", "用户名长度太短", "用户名长度超过16位",
					"您的用户名或密码错误", "超时，请重新登陆", "请输入密码", "密码长度小于6位", "密码中含有非法字符",
					"请输入用户名和密码", "密码不能超过16位", "请输入验证码", "验证码不能超过4个",
					"请输入用户名，密码和验证码", "验证码错误");
			...
			$('#authCode').blur(
					function() {
						var vcode = $(this).val();
						if (vcode.length > 0) {
							$.getJSON('/verification/checkVerifyCode/' + vcode,
									function(data) {
										if (!data.success) {
											$(".login-tip").html("");
						...
						if (data.success) {
							...
							if (data.neesRedirect) {
								if ($('#bid').val() != '') {
									...
									if(bid >= 500000){
										location.href = '/week/weekDetail/'
											+ $('#bid').val();
									}else{
										location.href = '/borrow/borrowDetail/'
											+ $('#bid').val();
									}
		...
		function changeImage() {
			$("#authCodeImg").attr(
					{
						"src" : "/verification/getGenImage/68/32?id="
								+ Math.random() * 1000,
						"alt" : "看不清，换一张"
					});
		}
