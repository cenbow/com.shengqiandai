	...
	$(".tx111").click(
			function() {
				...
				if ($("#msg").val() == 'noBindCard') {
					webUtil.arrowPrompt($("#addCard"), '请先绑定一张银行卡', 'err');
					return;
				}
				...
				if (checkWithdrawCash()) {
					...
					$.ajax({
						...
						success : function(data) {
							...
							art.dialog.tips(data.msg, 0);
							art.dialog({
								content:data.msg,
								ok:function(){
									location.href = '/accountCash/takeCash';
								},
								lock:true,
								icon:'succeed'
							});
						...
						beforeSend : function(XHR) {
							art.dialog.tips('提现申请提交中...', 1000);
							$('#tijiao').attr('disabled', true);
							$("#txForm input").attr('disabled', true);
						},
						complete : function(XHR, TS) {
							$("#txForm input").attr('disabled', false);
...
function checkWithdrawCash() {
	...
	if (withdrawCash == " " || withdrawCash == null) {
		...
		return false;
		/*
		 * } else if (withdrawCash > 500000) { $("#withdrawCashErr").html( '<i
		 * class="icons reg-error"></i>单次提现金额不能超过500000').show();
		 * $('#withdrawFee').html('0.00'); return false;
		 */
...
function getTakeCashInfo() {
	...
	var isIcbcCard = $("#isIcbcCard").val();
	$.post('/accountCash/getCashFee', {
		takeMoney : takeMoney,
		hongbao : hongbao,
		isIcbcCard : isIcbcCard
	}, function(data) {
		res = $.parseJSON(data);
		if (res.success) {
