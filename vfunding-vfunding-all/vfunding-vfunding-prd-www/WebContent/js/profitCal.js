...
//等本等息
function getData5() {
	var type = $.trim($("#repayType").val()); //还款方式
	var amount = $.trim($("#investAmount").val()); //金额
	var reg = /^[0-9]*[1-9][0-9]*$/;
	var newPar = /^\d+(\.\d+)?$/;
	if (!reg.test(amount)) {
		return;
	}
	var ann = $.trim($("#anun").val()); //年利率
	var count = $.trim($("#investTime").val()); //期限
	if (!newPar.test(ann)) {
		return;
	}
	if (!reg.test(count)) {
	...
	$("#earnBxNum").html(changeTwoDecimal_f(sumCapitalInterest)); //本息合计
	$("#lxNum").html(changeTwoDecimal_f(interest * count)); //利息收入
	$("#earnBxNumMonth").html(changeTwoDecimal_f(interest)); //每月收款
	$(".monthIncome").hide();
	var html = "<table><tr><th>月份</th><th>本金余额(￥)</th><th>本息余额 (￥)</th><th>月收本息 (￥)</th><th>月收本金(￥)</th><th>利息(￥)</th></tr></table>";
	html += '<div class="colle-table"><table><tr>';
	html += '<td></td>';
	...
	html += '<td></td>';
	html += '</tr></table></div>';
	var leftAccount = '0.00';
	var leftInterest = '0.00';
	for (var i = 0; i < count; i++) {
		...
		leftAccount = parseFloat(amount/count)*(i+1);
		leftInterest = parseFloat(interest)*(i+1);
		html += '<div class="colle-table"><table><tr>';
		html += '<td>' + (i + 1) + '</td>';
		html += '<td>' + changeTwoDecimal_f(amount-leftAccount) + '</td>';
		html += '<td>' + changeTwoDecimal_f(sumCapitalInterest-leftInterest-leftAccount) + '</td>';
		html += '<td>' + changeTwoDecimal_f(sumCapitalInterest/count) + '</td>';
		html += '<td>' + changeTwoDecimal_f(amount/count) + '</td>';
		html += '<td>' + changeTwoDecimal_f(interest) + '</td>';
		html += '</tr></table></div>';
		$("#loanTableBody").html(html).slideDown();
	}
}
