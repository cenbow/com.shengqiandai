...
$(function() {
	...
	/*
	 * tenderMoney .keyup(function() { var charge = $.trim(tenderMoney.val());
	 * if (isNaN(charge * 1) || charge * 1 <= 0) {
	 * tenderMoneyWrong.html('请输入正确的金额').show(); tenderMoney.val('');
	 * tenderMoney.focus(); return false; } else { var douhao = new RegExp(",",
	 * "g"); var n = ((parseFloat($('.tenderMoney').html().replace( douhao,
	 * ",")) || 0) * 1 + tenderMoney.val() * 1) .toFixed(2); var re =
	 * /^([1-9][\d]{0,7}|0)(\.[\d]{1,2})?$/;
	 * $('.tenderMoney').html(n.replace(re, '$1,')); tenderMoneyWrong.html('');
	 * return true; } });
	 */
...
function atonceInvest(zfPwd, tenderMoney) {
	...
	$.ajax({
		...
		data : {
			...
			"isUseHikes" : isUseHikes
		},
		success : function(data) {
			data = $.parseJSON(data);
			var status = data.status;
			var payMoney = data.payMoney;
			var realPayMoney = data.realPayMoney;
			var artMsg;
			// 1:全额投资,2:部分投资,3:已满标
			if (status == 1) {
				artMsg = "您成功投资" + payMoney + "元";
			} else if (status == 2) {
				artMsg = "由于标的已满,实际成功投资"+realPayMoney+"元。";
			} else if (status == 3) {
				artMsg = "此标已满,可选择其它标的";
			}
			art.dialog({
...
function tenderAll() {
	...
	var cashVolumeMoney = $("#cashVolumeMoney").val();

	if ($('#cashVolumeCheck').attr('checked') && cashVolumeMoney > 0) {
		useMoney = parseFloat(useMoney) + parseFloat(cashVolumeMoney);
	}
