...
public class ExportController {
	 ...
	 */
	@NeedSession
	@RequestMapping(value="/tenderExcel")
	public void exportExcel(HttpServletResponse response,HttpServletRequest request,
			QueryParameterVO vo) throws Exception{
		PageSearch pageSearch = new PageSearch();
		pageSearch.setEntity(vo);
		ModelAndViewUtil.addQueryTimeVO(vo);
		String filename = "投资记录";
		...
		try {
			...
			pageSearch.setEntity(vo);
			pageSearch.setRows(1000);//默认导出200条
			if(vo.getType()==1){ //投标项目
				WritableSheet sheet = workbook.createSheet("投标列表", 0); // 投标列表
				...
				label = new jxl.write.Label(0, 0, "序号",wcf);
				sheet.addCell(label);
				label = new jxl.write.Label(1, 0, "项目名称",wcf);
				sheet.addCell(label);
				label = new jxl.write.Label(2, 0, "利率",wcf);
				sheet.addCell(label);
				label = new jxl.write.Label(3, 0, "期限",wcf);
				sheet.addCell(label);
				label = new jxl.write.Label(4, 0, "投资金额",wcf);
				sheet.addCell(label);
				label = new jxl.write.Label(5, 0, "预期纯收益",wcf);
				sheet.addCell(label);
				label = new jxl.write.Label(6, 0, "待收金额",wcf);
				sheet.addCell(label);
				label = new jxl.write.Label(7, 0, "已收金额",wcf);
				sheet.addCell(label);
				label = new jxl.write.Label(8, 0, "状态",wcf);
				sheet.addCell(label);
				
				List<BorrowTenderVO> list = borrowTenderService.selectInvestRecordListPage(pageSearch);//投标集合
				BigDecimal all = new BigDecimal("0");
				for (int i = 1; i <=list.size(); i++) {
					BorrowTenderVO btvo = list.get(i-1);
					BigDecimal b = btvo.getApr().subtract(btvo.getBfee()==null?new BigDecimal("0"):btvo.getBfee()).subtract(btvo.getGfee()==null?new BigDecimal("0"):btvo.getGfee());
					label = new jxl.write.Label(0, i, ""+i,wcf2);
					sheet.addCell(label);
					label = new jxl.write.Label(1, i, btvo.getBorrowName(),wcf2);
					sheet.addCell(label);
					label = new jxl.write.Label(2, i, btvo.getApr().toString()+"%",wcf2);
					sheet.addCell(label);
					label = new jxl.write.Label(3, i, btvo.getTimeLimit().toString()+"个月",wcf2);
					sheet.addCell(label);
					label = new jxl.write.Label(4, i, btvo.getAccount().toString(),wcf2);
					sheet.addCell(label);
					label = new jxl.write.Label(5, i, b.toString()+"%",wcf2);
					sheet.addCell(label);
					label = new jxl.write.Label(6, i, btvo.getRepaymentAccount(),wcf2);
					sheet.addCell(label);
					label = new jxl.write.Label(7, i, btvo.getRepaymentYesAccount(),wcf2);
					sheet.addCell(label);
					label = new jxl.write.Label(8, i, btvo.getStatus()==1?"投资成功":(btvo.getStatus()==2?"失败":"待审"),wcf2);
					sheet.addCell(label);
					all = all.add(new BigDecimal(btvo.getAccount()));
				}
				label = new jxl.write.Label(0, pageSearch.getTotalResult()+2, "共有 "+pageSearch.getTotalResult()+" 条记录  投资金额合计："+all);
				sheet.addCell(label);
			}
			
			if(vo.getType() ==2){ //待收项目
				WritableSheet sheet2 = workbook.createSheet("待收记录", 1); // 待收列表 
				sheet2.setColumnView(0, 15);
				sheet2.setColumnView(1, 12);
				sheet2.setColumnView(2, 12);
				sheet2.setColumnView(3, 15);
				sheet2.setColumnView(4, 15);
				sheet2.setColumnView(5, 15);
				...
				label = new jxl.write.Label(1, 0, "项目名称",wcf);
				sheet2.addCell(label);
				label = new jxl.write.Label(2, 0, "应收时间",wcf);
				sheet2.addCell(label);
				label = new jxl.write.Label(3, 0, "期次",wcf);
				sheet2.addCell(label);
				label = new jxl.write.Label(4, 0, "应收本息",wcf);
				sheet2.addCell(label);
				label = new jxl.write.Label(5, 0, "应收利息",wcf);
				sheet2.addCell(label);
				label = new jxl.write.Label(6, 0, "应收金额",wcf);
				sheet2.addCell(label);
				label = new jxl.write.Label(7, 0, "状态",wcf);
				sheet2.addCell(label);
				
				vo.setStatus(vo.getStatus()==1?-1:(vo.getStatus()==2?-2:-12));
				pageSearch.setEntity(vo);
				List<CollectionTenderUserVO> resultList = this.borrowCollectionService.selectCollectionRecordListPage(pageSearch);//待收集合
				BigDecimal all2 = new BigDecimal("0");
				for (int i = 1; i <=resultList.size(); i++) {
					CollectionTenderUserVO uvo = resultList.get(i-1);
					label = new jxl.write.Label(0, i, ""+i,wcf2);
					sheet2.addCell(label);
					label = new jxl.write.Label(1, i, uvo.getBorrowName(),wcf2);
					sheet2.addCell(label);
					label = new jxl.write.Label(2, i, new SimpleDateFormat("yyyy-MM-dd").format(new Date(Long.parseLong(uvo.getRepayTime()+"000"))).toString(),wcf2);
					sheet2.addCell(label);
					label = new jxl.write.Label(3, i, (uvo.getOrder()+1)+"",wcf2);
					sheet2.addCell(label);
					label = new jxl.write.Label(4, i, uvo.getRepaymentAccount().toString(),wcf2);
					sheet2.addCell(label);
					label = new jxl.write.Label(5, i, uvo.getInterest().toString(),wcf2);
					sheet2.addCell(label);
					label = new jxl.write.Label(6, i, uvo.getCapital().toString(),wcf2);
					sheet2.addCell(label);
					label = new jxl.write.Label(7, i, uvo.getStatus()==1?"已还":"未还",wcf2);
					sheet2.addCell(label);
					all2 = all2.add(uvo.getRepaymentAccount());
				}
				label = new jxl.write.Label(0, pageSearch.getTotalResult()+2, "共有 "+pageSearch.getTotalResult()+" 条记录  待收金额合计："+all2);
				sheet2.addCell(label);
			}
	 ...
	 * @throws IOException 
	 */
	public void createPdf(HttpServletResponse response,Borrow borrow,User user,List<BorrowTenderVO> tenderList) throws IOException{
		// 创建一个Document对象
        ...
        //	BaseFont bfChinese = BaseFont.createFont("STSongStd-Light", "UniGB-UCS2-H", false);  
            ...
            String[] content = { " 本《债权转让协议》（“本协议”）由以下双方于 "+
                    ...
                    "双方根据平等、自愿的原则，就甲方通过由容塔金融信息服务（上海）有限公司（“微积金”）运营管理的微积金网",
	...
	public void createPdf2(HttpServletResponse response,Borrow borrow,User user,List<BorrowTenderVO> tenderList) throws IOException{
		...
		try {  
			...
			String[] content = { "借款人（甲方）：  "+user.getUsername(),  
					...
					"遵照自愿、平等的原则，就甲方通过由容塔金融信息服务（上海）有限公司（“微积金”）运营管理的微积金网（域名为www.vfunding.cn，“微积金网”）",
